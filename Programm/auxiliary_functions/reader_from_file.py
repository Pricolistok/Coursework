...
def read_lights_from_file(filename: str):
    """
    Считывает источники света из файла.
    Формат строки: DirX DirY DirZ R G B Intensity
    """
    lights = []
    try:
...
def reader_directions_from_file(filename):
    """
    Считывает матрицу направлений из файла.
    Возвращает список списков (int).
    """
    matrix = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
