...
def rasterize_face_with_shadows(face, zbuffer, img_array, cam, shadow_casters, current_scale):
    ...
    # Подготовка данных для КАЖДОГО источника света
    # Список [ (diffuse_factor, shadow_coords_for_triangle), ... ]
    casters_data = []
    for caster in shadow_casters:
        ...
        # Координаты тени (преобразуем треугольник в пространство этого источника)
    ...
    for y in range(min_y, max_y + 1):
        ...
        for i in range(n_verts):
            ...
            if (y1 <= y < y2) or (y2 <= y < y1):
                ...
                # Интерполяция координат теней для ВСЕХ источников
                # sc_list будет списком кортежей (sx, sy, sd) для каждого источника
                sc_list = []
                for c_data in casters_data:
                    s_poly = c_data['s_coords']  # массив вершин в light space
                    s1 = s_poly[i]
                    s2 = s_poly[(i + 1) % n_verts]
                    # Интерполяция (sx, sy, sd)
                    s_int = (
                        s1[0] + t * (s2[0] - s1[0]),
                        s1[1] + t * (s2[1] - s1[1]),
                        s1[2] + t * (s2[2] - s1[2])
        ...
        for k in range(0, len(intersections) - 1, 2):
            ...
            # --- РАСЧЕТ ИНТЕНСИВНОСТИ (СУММА ВСЕХ ИСТОЧНИКОВ) ---
            # Начинаем с базового фонового света
            ...
            for idx, c_data in enumerate(casters_data):
                ...
                # Достаем интерполированные координаты тени для ЭТОГО источника
                # left[2] - это sc_list левой точки, right[2] - правой
                # Берем [idx] элемент списка
                sl = np.array(left[2][idx])
                sr = np.array(right[2][idx])

                # Интерполяция внутри линии
                ...
                # Вклад света (по умолчанию 0, если в тени)
...
def draw_scene_with_objects(label_field, faces, cam, current_scale, objects=[]):
    ...
    # 1. Рендерим карты теней для ВСЕХ источников
    if casters:
